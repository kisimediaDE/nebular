...
`@kisimedia/nebular-firebase-auth` allows authentication in firebase applications with `@kisimedia/nebular-auth`.
...
`@kisimedia/nebular-auth` package is sponsored by [GO-ER](https://go-er.com) and [What Now Travel](https://whatnow.travel/).
...
```sh
  npm i @kisimedia/nebular-auth @kisimedia/nebular-firebase-auth
```
...
import { NbFirebasePasswordStrategy } from '@kisimedia/nebular-firebase-auth';
