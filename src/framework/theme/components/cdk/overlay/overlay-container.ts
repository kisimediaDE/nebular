...
export class NbOverlayContainerComponent {
  ...
  attachComponentPortal<T>(portal: NbComponentPortal<T>, context?: Object): ComponentRef<T> {
    // ggf. eigenen Injector erzeugen, aber ohne ComponentFactoryResolver
    portal.injector = this.createChildInjector(this.injector);
  ...
  protected createChildInjector(parentInjector: Injector): NbPortalInjector {
    return new NbPortalInjector(parentInjector, new WeakMap());
  }
}
