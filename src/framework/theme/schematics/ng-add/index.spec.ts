...
const EXPECTED_THEME_SCSS = `@forward '@kisimedia/nebular-theme/styles/theming';
...
describe('ng-add', () => {
  ...
  describe('ng-add', () => {
    ...
    it('should add @kisimedia/nebular-theme in package.json', function () {
      const dependencies = getPackageDependencies(tree);
      const nebularThemeVersion = require('../../package.json').version;

      expect(dependencies['@kisimedia/nebular-theme']).toBeDefined();
      expect(dependencies['@kisimedia/nebular-theme']).toBe(nebularThemeVersion);
    });

    it('should add @kisimedia/nebular-eva-icons in package.json', function (done) {
      let dependencies = getPackageDependencies(tree);
      const nebularEvaIconsVersion = require('../../package.json').version;

      expect(dependencies['@kisimedia/nebular-eva-icons']).toBeDefined();
      expect(dependencies['@kisimedia/nebular-eva-icons']).toBe(nebularEvaIconsVersion);
  ...
  it('should register inline theme if no theme already registered', (done) => {
    runSetupSchematic({ customization: false }).subscribe(async (tree) => {
      ...
      expect(styles).toContain('./node_modules/@kisimedia/nebular-theme/styles/prebuilt/default.css');
      done();
    });
  });
